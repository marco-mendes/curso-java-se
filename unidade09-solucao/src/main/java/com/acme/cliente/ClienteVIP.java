...
public class ClienteVIP extends Cliente {
	...
	public Map<String, Conta> coletaContasComUsoLimiteCredito() {
		return contas.entrySet().stream().
			   filter(e -> e.getValue().getSaldo() <=0).
	...
	public static void main(String[] args) throws MovimentacaoInvalida, LimiteSaqueExcedido,
		...
		maria.adiconarNovaConta("34567-6", 0);
		maria.debitar("34567-6", 100);
		maria.adiconarNovaConta("89734-6", 0);
		maria.debitar("89734-6", 100);
		System.out.println(maria);
		System.out.println("Saldo acumulado das contas da Maria: " + maria.getSaldo());

		System.out.println("Contas da Maria no cheque especial: ");

		Map<String, Conta> contasNegativas = maria.coletaContasComUsoLimiteCredito();
        contasNegativas.values().forEach(conta -> System.out.println(conta));
